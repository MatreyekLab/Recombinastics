...
```{r Initial validation data for G542Ac3 and G783Ac6 cells repeated}
...
```{r Initial validation data for G542Ac3 and G783Ac11 cells repeated}
...
```{r G542Ac3 and G783Ac11 cell plots for manuscript}
...
```{r PCA on all datapoints from all replicates from all clones}
...
```{r Representative GcaMP plot with negative control}
rep_4b = ggplot() +
...
Supplementary Figure 3 - un-normalized fluorescence measurement curves for variants in carbachol and ionomycin
```{r Raw GCaMP curves for supplement}
...
Figure 4C - Differences in initial vs peak fluorescence for each variant for carbachol and ionomycin
```{r GCaMP initial and peak comparisons}
#100uM carbachol 
#replicate 1
...
```{r Normalized GCaMP data for plot}
#carbachol
#rep 1
...
```{r Import the vif orientations data}
...
```{r Individual Vif-Apobec data for supplementary figure}
...
```{r Importing GN sequencing data}
...
```{r Recombination mixture experiment plot}
...
```{r Modeling predicted pairwise specificities based on the GN data}
